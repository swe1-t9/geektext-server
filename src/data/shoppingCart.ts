...
const saveShoppingCart = async (userid: string): Promise<saved_carts> => {
  ...
  await db('saved_cart_items').insert(items);
...
// moved all the items in a user's shopping cart to their saved cart
const checkoutUser = async (userid: string): Promise<shopping_carts> => {
  // first, fetch the cart id from the user id
  const shoppingCart = await db('shopping_carts')
    .select('*')
    .where({ user_id: userid })
  ...
  await db('sold_items').insert(items);
